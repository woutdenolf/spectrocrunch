...
class CompoundFromElement(compoundfromformula.CompoundFromFormula):
    ...
    def __init__(self, el, oxidationstate=None, typ="oxide", defaultoxidationstate="max"):
        ...
        if oxidationstate is None:
            if len(cmpds) == 1:
                ...
                if defaultoxidationstate == "max":
                    ...
                    raise RuntimeError(
                        "Select one of the available {} oxidations states: {}".format(el, cmpds.keys()))
            ...
            raise RuntimeError(
                "Oxidation state {} does not exist for {} {}".format(oxidationstate, el, typ))
        ...
        parse = re.compile(
            "^(?P<Z>[A-Z][a-z]?)(?P<a>\d?)(?P<X>[A-Z][a-z]?)(?P<b>\d?)$")
