...
class test_pymca(unittest.TestCase):
    ...
    @unittest.skipIf(xrfdetectors.compoundfromname.xraylib is None, "xraylib not installed")
    def test_rates(self):
        h = xrf_setup.simple(energy=self.energy, escape=0,
                             snip=False, continuum=1)
        ...
        for group in fitresult["fitareas"]:
            ...
            grouprate_avg = h.mcafit._fluoRates[0][element]["rates"]["{} xrays".format(
                linegroup)]
            ...
            for j in range(h.sample.nlayers):
                ...
                if element in h.mcafit._fluoRates[i]:
                    ...
                    grouprate_l = h.mcafit._fluoRates[i][element]["rates"]["{} xrays".format(
                        linegroup)]
            ...
            for j in range(h.sample.nlayers):
                ...
                if element in h.mcafit._fluoRates[i]:
                    ...
                    np.testing.assert_allclose(
                        fitresult["lmassfractions"][j][group], fitresult["fitareas"][group]/(h.I0*grouprate))
