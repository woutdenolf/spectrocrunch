...
class test_lut(unittest.TestCase):
    ...
    def test_interpolate(self):
        ...
        def func(x): return l1(units.Quantity(x, 'keV')).to('mm').magnitude
        s = str(l1)
    ...
    def test_add(self):
        ...
        l2 = lut.LUT(x=x, y=y)
        l3 = l1+l2
        def func(x): return l3(units.Quantity(
            x, 'keV')).to('mA').magnitude.tolist()
    ...
    def test_replace(self):
        ...
        def func(x): return l1(units.Quantity(
            x, 'keV')).to('mA').magnitude.tolist()
