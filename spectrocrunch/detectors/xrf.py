...
class XRFDetector(with_metaclass(base.CentricCone)):
    ...
    def __eq__(self, other):
        if isinstance(other, self.__class__):
            ...
            if self.fixedarearatios != other.fixedarearatios:
                return False
            if self.fixedarearatios:
                if self.stailbroadening != other.stailbroadening or \
                   self.ltailbroadening != other.ltailbroadening or \
                   self.fractions != other.fractions:
                    return False
            else:
                if self.stailslope_ratio != other.stailslope_ratio or \
                   self.ltailslope_ratio != other.ltailslope_ratio or \
                   self.ratios != other.ratios:
                    return False
            return self.mcazero == other.mcazero and \
                self.mcagain == other.mcagain and \
                self.mcanoise == other.mcanoise and \
                self.mcafano == other.mcafano and \
                self.shape_conversionenergy == other.shape_conversionenergy and \
                self.bpeak == other.bpeak and \
                self.bstail == other.bstail and \
                self.bltail == other.bltail and \
                self.bstep == other.bstep
        else:
            return False
    ...
    def stailbroadening(self, value):
        ...
        if self._calc_fixedarearatios:
            self.stailslope_ratio = self._calc_tail_slope_ratio(
                self.stailbroadening)
    ...
    def ltailbroadening(self, value):
        ...
        if self._calc_fixedarearatios:
            self.ltailslope_ratio = self._calc_tail_slope_ratio(
                self.ltailbroadening)
    ...
    def stailslope_ratio(self, value):
        ...
        if self._calc_notfixedarearatios:
            self.stailbroadening = self._calc_tail_boardening(
                self.stailslope_ratio)
    ...
    def ltailslope_ratio(self, value):
        ...
        if self._calc_notfixedarearatios:
            self.ltailbroadening = self._calc_tail_boardening(
                self.ltailslope_ratio)
    ...
    def _calc_ratios(self, energy=None):
        ...
        return self._fcalc_ratios_single(self.all_fractions, _str/stnorm, ltr/ltnorm, gnorm/snorm)
    ...
    def _fcalc_fractions_single(self, ratios, cstail, cltail, cstep):
        ...
        if self.bpeak:
            A = np.array([[rstail+cstail, rstail, rstail], [rltail,
                                                            rltail+cltail, rltail], [rstep, rstep, rstep+cstep]])
    ...
    def _snorm(self, u, a, b, approx=False):
        ...
        if approx:
            ...
            zero = a*np.exp(-u**2/a)/(2*np.sqrt(np.pi)) - \
                u*scipy.special.erfc(u/b)/2.
            snorm = u + zero
        return snorm
    ...
    def _tnorm(self, u, a, b, tr, approx=False):
        ...
        if approx:
            ...
            minusone = np.exp(
                a/(4.*tr**2)-u/tr) * scipy.special.erfc((a/(2.*tr)-u)/b) + scipy.special.erf(-u/b)
        ...
        for btail, tail_H, tailslope_ratio in zip([bstail, bltail], [stail_H, ltail_H], [stailslope_ratio, ltailslope_ratio]):
            if onlyheight:
                ...
                if btail:
                    with np.errstate(over="ignore"):
                        mexp = tail_H/2. * \
                            np.exp(diff/tailslope_ratio+gvar /
                                   (2.*tailslope_ratio**2))
                        ind = np.isinf(mexp)
                        if ind.any():
                            mexp[ind] = 0  # lim_x->inf exp(x)*erfc(x) = 0
                    ...
                    yti = mexp * \
                        scipy.special.erfc(argstep+b/(2.*tailslope_ratio))
                else:
    ...
    def addtopymca(self, setup, cfg):
        ...
        bltail = self.bltail
        bstep = self.bstep
        cfg["fit"]["hypermetflag"] = 1 * \
            bhypermet | 2*bstail | 4*bltail | 8*bstep
    ...
    def loadfrompymca(self, setup, cfg):
        ...
        # Compensate fano for difference in Ehole with pymca's 3.85 eV (ClassMcaTheory)
        self.mcafano = cfg["detector"]["fano"] * \
...
class sn3102(XRFDetector):
    ...
    def __init__(self, **kwargs):
        ...
        attenuators["WindowDetector"] = {
            "material": moxtek, "thickness": 380e-4}  # AP3.3 Moxtek
...
class Leia(XRFDetector):
    ...
    def __init__(self, **kwargs):
        ...
        attenuators["FoilDetector"] = {
            "material": ultralene, "thickness": 4.064e-4}  # cm
        attenuators["WindowDetector"] = {
            "material": element.Element('Be'), "thickness": 25e-4}
        attenuators["Detector"] = {
...
class BB8(XRFDetector):
    ...
    def __init__(self, **kwargs):
        ...
        attenuators["FoilDetector"] = {
            "material": ultralene, "thickness": 4.064e-4}  # cm
        attenuators["WindowDetector"] = {
            "material": element.Element('Be'), "thickness": 12.5e-4}
        attenuators["Detector"] = {
            "material": element.Element('Si'), "thickness": 450e-4}
        kwargs["attenuators"] = attenuators
...
class ID16b_Virtual1(XRFDetector):
    ...
    def __init__(self, **kwargs):
        attenuators = kwargs.get("attenuators", {})
        attenuators["WindowDetector"] = {
            "material": element.Element('Be'), "thickness": 50e-4}
        attenuators["Deadlayer"] = {
            "material": element.Element('Si'), "thickness": 30e-7}
        attenuators["Detector"] = {
            "material": element.Element('Si'), "thickness": 450e-4}
        kwargs["attenuators"] = attenuators
