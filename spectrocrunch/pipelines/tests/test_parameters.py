...
class test_parameters(unittest.TestCase):
    ...
    def hashequal(self, a, b):
        self.assertEqual(hashing.calchash(a),
                         hashing.calchash(b))

    def hashnotequal(self, a, b):
        self.assertNotEqual(hashing.calchash(a),
                            hashing.calchash(b))
    ...
    def test_hash(self):
        ...
        # Numbers:
        self.hashequal(np.float32(f1), np.float64(f1))
        self.hashequal(float("1.23e-4"), 1.23e-4)

        # Dictionaries
        self.hashequal({'a': f1, 'b': f2}, {'a': f1, 'b': f2})
        self.hashnotequal(
            collections.OrderedDict(zip(['a', 'b'], [f1, f2])),
            collections.OrderedDict(zip(['b', 'a'], [f2, f1]))
        )
    ...
    def _parameters(self, n):
        ...
        if n > 26:
            letters = list(itertools.combinations_with_replacement(
                letters, (n+26-1)//26))
        letters = letters[:n]
    ...
    def _generate_parameters(self, n=100):
        ...
        params_local = [self._parameters_sample(
            superdict, n//2, overwrite=False)]
        n = 2*n//3
        for i in range(n):
    ...
    def _shuffle_nodes(self, nodes):
        ...
        for i in range(len(nodes)*2):
            ...
            if before:
                _, newnode = nodes[index_dest].insert_before(
                    node=nodes[index_source], single=True)
            else:
                _, newnode = nodes[index_dest].insert_after(
                    node=nodes[index_source], single=True)
    ...
    def _checknodes(self, nodes):
        ...
        lst |= set(id(node) for node in nodes if id(
            node) != id(root) and not node.children)
        self.assertEqual(len(lst), len(nodes))
    ...
    def test_insert_connections(self):
        for nlevels in [1, 2, 4, 10]:
            ...
            for i in range(nlevels):
                nodeslast = [node.branch(name="Node{}".format(ctr()))
                             for node in nodeslast for i in range(2)]
    ...
    def _random_tree(self, name="Node", nlevels=2):
        ...
        ctr = Counter(1)
        for i in range(nlevels):
            nodeslast = [node.branch(name="{}{}".format(name, ctr()))
                         for node in nodeslast for i in range(2)]
            nodes.extend(nodeslast)
