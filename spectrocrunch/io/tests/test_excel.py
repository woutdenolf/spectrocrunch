...
class test_excel(unittest.TestCase):
    ...
    def _generate_df(self, writer=None):
        data = np.random.random(
            self.nrow*self.ncol).reshape((self.nrow, self.ncol))
        priority = random.choice(list(excel.DataFrame.priorities))
        df = excel.DataFrame(writer=writer, data=data, columns=self.columns(
        ), index=self.rows(), priority=priority)
        for i in range(5):
    ...
    def test_save(self):
        ...
        with excel.Writer(filename) as writer:
            ...
            df1.save()
        df2 = next(iter(excel.DataFrame.fromexcel(
            filename, index_col=0).values()))
    ...
    def test_dataframe(self):
        ...
        for equal in [True, False]:
            ...
            df1 = excel.DataFrame(
                columns=columns, index=rows, priority=priority)
            ...
            i = 2
            df1.addcell_formula(rows[i], columns[i], "{}",
                                rows[self.nrow-1], columns[self.ncol-1])
            df2.loc[rows[i], columns[i]
                    ] = df2.loc[rows[self.nrow-1], columns[self.ncol-1]]
            ...
            if equal:
                ...
                np.testing.assert_raises(
                    AssertionError, np.testing.assert_array_equal, df1.df.values, df2.values)
