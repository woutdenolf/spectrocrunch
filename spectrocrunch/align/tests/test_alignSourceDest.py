...
class test_alignSource(unittest.TestCase):
    ...
    def test_sourcedest(self):
        ...
        listofstacks2 = [np.arange(8, dtype=dtype).reshape(
            2, 2, 2) for s in listofstacks]
        listofstacks3 = [np.empty(s.shape, dtype=s.dtype)
                         for s in listofstacks]
        listofstacks4 = [np.zeros(s.shape, dtype=s.dtype)
                         for s in listofstacks]
        ...
        with h5py.File(absh5name) as h5f, h5py.File(absh5name2) as h5f2, h5py.File(absh5name3) as h5f3, h5py.File(absh5name4) as h5f4:
            ...
            for i in range(nstacks):
                ...
                h5f2.create_dataset(
                    names[i], data=listofstacks2[i], maxshape=shape)
            ...
            edfnames = [[form % (name, i) for i in range(nimages)]
                        for name in names]
            absedfnames = [[os.path.join(self.dir.path, 'edfdir', form % (
                name, i)) for i in range(nimages)] for name in names]
            absedfnames2 = [[os.path.join(self.dir.path, 'edfdir2', form % (
                name, i)) for i in range(nimages)] for name in names]
            absedfnames3 = [[os.path.join(self.dir.path, 'edfdir3', form % (
                name, i)) for i in range(nimages)] for name in names]
            absedfnames_1 = [[os.path.join(self.dir.path, 'edfdir_1', form % (
                name, i)) for i in range(nimages)] for name in names]
            ...
            self.dir.compare(
                tuple([item for sublist in edfnames for item in sublist]), path='edfdir2')
            ...
            for i in range(nstacks):
                for j in range(nimages):
                    ...
                    for s in source:
                        ...
                        np.testing.assert_array_equal(s.readimgas(
                            i, j, np.float32), img.astype(np.float32))
            ...
            dest = [alignDest(absh5name, names, "", stackdim=stackdim),  # adds an entry1 subdirectory
                    alignDest(absh5name2, names, "", stackdim=stackdim,
                    ...
                    alignDest(absh5name3, names, ".aligned",
                              stackdim=stackdim),
                    alignDest(os.path.join(self.dir.path, 'edfdir'), names,
                              ".edf", stackdim=stackdim),  # adds a ..._1 subdirectory
                    alignDest(os.path.join(self.dir.path, 'edfdir2'), names, ".edf",
                              stackdim=stackdim, overwrite=True),  # overwrites the files
                    # creates files in the existing (empty) directory
                    alignDest(os.path.join(self.dir.path, 'edfdir3'),
                              names, ".edf", stackdim=stackdim),
                    alignDest(listofstacks2, None, None, stackdim=stackdim,
            ...
            self.dir.compare(
                tuple([item for sublist in edfnames for item in sublist]), path='edfdir')
            self.dir.compare(
                tuple([item for sublist in edfnames for item in sublist]), path='edfdir_1')
            self.dir.compare(
                tuple([item for sublist in edfnames for item in sublist]), path='edfdir2')
            self.dir.compare(
                tuple([item for sublist in edfnames for item in sublist]), path='edfdir3')

            # Check written content
            h5datasets3_a = [h5f3["/entry1/"+name] for name in names]
            h5datasets3_b = [h5f3[name+".aligned"] for name in names]
            h5datasets3_c = [h5f3["/entry1/"+name+".aligned"]
                             for name in names]
            h5datasets3_d = [h5f3["/entry2/"+name+".aligned"]
                             for name in names]
            for i in range(nstacks):
                for j in range(nimages):
                    img = np.take(listofstacks_readonly[i], j, axis=stackdim)
                ...
                np.testing.assert_array_equal(
                    listofstacks_readonly[i], h5datasets[i])
                np.testing.assert_array_equal(
                    listofstacks_readonly[i], h5datasets2[i])
                np.testing.assert_array_equal(
                    listofstacks_readonly[i], h5datasets3[i])
                np.testing.assert_array_equal(
