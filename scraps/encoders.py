...
def getcoord(header, scanlabel="", fastlabel="", slowlabel=""):
    ...
    if scaninfo["name"] == "zapimage":
        sfast = {
            "name": scaninfo["motfast"],
            "data": spec.zapline_values(
                scaninfo["startfast"], scaninfo["endfast"], scaninfo["npixelsfast"]
            ),
        }
        sslow = {
            "name": scaninfo["motslow"],
            "data": spec.ascan_values(
                scaninfo["startslow"], scaninfo["endslow"], scaninfo["nstepsslow"]
            ),
        }
...
def getpos(path, sample, scannr, zapctr):
    f = os.path.join(
        path, sample, "zap", "{}_arr_{}_{:04}_0000.edf".format(sample, zapctr, scannr)
    )
    f = fabio.open(f)
...
def getencoder(path, sample, scannr, mot):
    f = os.path.join(
        path, sample, "zap", "{}_arr_{}_{:04}_0000.edf".format(sample, mot, scannr)
    )
    f = fabio.open(f)
    return f.data
