...
def attzscan(specfile,scannumber,outpath,withsample=False,iodettype=2):
    ...
    # Data
    f = spec(specfile)
    result = f.getdata2(scannumber, ["attz","iodet","idet","Seconds","Photons","Storage Ring A"])
    ...
    attfoil = np.empty(n)
    for i in range(n):
        attfoil[i] = np.exp(-comp[i].mass_att_coeff(energy)[0]*comp[i].density*thickness[i])
    ...
    if withsample:
        attsample = np.exp(-ultralene.mass_att_coeff(energy)[0]*ultralene.density*(4*1e-4))
    else:
        attsample = 1
    ...
    plt.plot(photons,b0+g0*photons/(attiodet*attsample),color=p[0].get_color())
    plt.xlabel("Photons")
    plt.ylabel("iodet")
    plt.legend(loc=2)
    ...
    plt.plot(attzi,b0 + g0*I0*attfoil,'o',label='iodet(I0)',color=p[0].get_color())
    p = plt.plot(attz,idet,label='idet')
    plt.plot(attzi,b + g*I0*attfoil*attiodet*attsample,'o',label='idet(I0)',color=p[0].get_color())
    plt.xlabel("attz")
    plt.title("{}: {}".format(specfile,scannumber))
    plt.legend()
    fig.savefig(os.path.join(outpath,'{}.png'.format(scannumber)),bbox_inches='tight',dpi=300)
