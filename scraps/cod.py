...
db = MySQLDatabase("cod", host="127.0.0.1", user="cod_reader", port=3308, connect_timeout=10000)
...
class Data(BaseModel):
    ...
    class Meta:
        db_table = 'data'
        indexes = (
            (('mineral', 'chemname', 'commonname'), False),
        )

    def __str__(self):
        ret = "{} ({})\n".format(self.mineral,self.commonname)
        ret += "{} ({})\n".format(self.formula,self.chemname)
        ret += "{} ({} {} {} {} {} {})\n".format(self.sg,self.a,self.b,self.c,self.alpha,self.beta,self.gamma)
    ...
    def satp(self):
        return self.sap(self.diffrpressure) and\
               self.sap(self.cellpressure) and\
               self.sat(self.diffrtemp) and\
               self.sat(self.celltemp)
    ...
    def namequery(cls,name):
        return cls.select().where(cls.mineral == name or cls.commonname == name or cls.chemname == name).order_by(cls.year.desc())
