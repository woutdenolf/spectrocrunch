...
def approach1():
    ...
    chi = np.arctan(np.tan(2*beta)*np.cos(delta))/2.
...
def approach2():
    ...
    BB = -2*np.cos(delta)/(meanabsphasor**2*(np.sin(delta))**2 *sinbeta*cosbeta)
    ...
    chi2 = np.arctan(tan2beta*np.cos(delta))/2.
    ...
    semimajor = meanabsphasor*np.sqrt((1+np.sqrt(1-(sin2beta*np.sin(delta))**2))/2.)
    semiminor = meanabsphasor*np.sqrt((1-np.sqrt(1-(sin2beta*np.sin(delta))**2))/2.)

    if delta!=0:
        np.testing.assert_allclose(AA*urx**2+BB*urx*ury+CC*ury**2,1)
        np.testing.assert_allclose(semimajor,  np.sqrt( 2./(AA+CC-np.sqrt((AA-CC)**2+BB**2)) )  )
        np.testing.assert_allclose(semiminor,  np.sqrt( 2./(AA+CC+np.sqrt((AA-CC)**2+BB**2)) )  )
